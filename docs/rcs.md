...
<CardGroup cols={3}>
  <Card title="Pairing" icon="link" href="https://docs.openclaw.ai/channels/pairing">
    Default DM policy for RCS is pairing.
  </Card>
  <Card title="Gateway security" icon="shield" href="https://docs.openclaw.ai/gateway/security">
    Review webhook exposure and sender access controls.
  </Card>
  <Card title="Channel troubleshooting" icon="wrench" href="https://docs.openclaw.ai/channels/troubleshooting">
...
- **Delivery and read receipts.** Outbound status callbacks progress through `sent`, `delivered`,
  and `read` (post-delivery read receipts arrive as `EventType=READ`), so the agent can know a
  message was actually seen instead of guessing from silence.
...
Exactly one channel can own the public shared path. The Gateway route registry accepts a single owner per exact path, so `channels.sms.webhookPath` must not equal `channels.rcs.sharedWebhookPath` — otherwise startup order decides which channel handles the shared Twilio URL. Keep SMS on a distinct internal path (`smsForwardWebhookPath`) as shown above. The RCS channel refuses to start a shared-webhook account whose `sharedWebhookPath` equals its own `webhookPath` or its `smsForwardWebhookPath`.
...
RCS allows up to approximately 3072 characters per message body. OpenClaw splits long agent replies before sending. The default chunk limit is 1600 characters (SMS-safe); raise it for RCS:
...
RCS output supports plain text. OpenClaw strips markdown, flattens fenced code blocks, preserves readable links, and splits long replies at the configured `textChunkLimit` before sending through Twilio.
