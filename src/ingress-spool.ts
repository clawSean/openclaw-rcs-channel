...
function parseRcsIngressForm(
  ...
  if (message.accountSid !== account.accountSid) {
    throw new RcsIngressPermanentError("RCS ingress payload has an invalid Twilio account.");
  }
  if (message.messagingServiceSid !== account.messagingServiceSid) {
    throw new RcsIngressPermanentError("RCS ingress payload has an invalid Messaging Service.");
  }
  return message;
}
...
export function createRcsIngressSpool(params: {
  ...
  >({
    ...
    // The pump can finish after claiming an event while its delivery remains active.
    // Give the delivery its own admitted root so agent-queue work does not inherit
    // the pump's released async context after the webhook acknowledgement.
