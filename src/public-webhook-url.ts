...
const RCS_STATUS_CALLBACK_MAX_LENGTH = 4_000;
const RCS_STATUS_CALLBACK_READ_TIMEOUT_MS = 5_000;
...
export function parseRcsPublicWebhookUrl(value: string): URL | undefined {
  const trimmed = value.trim();
  if (
    !ABSOLUTE_HTTPS_URL_PATTERN.test(trimmed) ||
  ...
  if (url.protocol !== "https:" || url.username || url.password || !hasValidHostname(url)) {
    return undefined;
  }
  return url;
}
...
export function resolveRcsStatusCallbackUrl(publicWebhookUrl: string): string {
  ...
  if (!trimmed || !parseRcsPublicWebhookUrl(trimmed)) {
    return "";
  }
  ...
  if (!normalizedPolicies.has("all")) {
    for (const requiredPolicy of ["ct", "rt", "5xx"]) {
      if (!normalizedPolicies.has(requiredPolicy)) {
  ...
  const readTimeout =
    ...
    configuredReadTimeout <= TWILIO_READ_TIMEOUT_MAX_MS
      ...
      : RCS_STATUS_CALLBACK_READ_TIMEOUT_MS;
  ...
  if (
    callbackUrl.length > RCS_STATUS_CALLBACK_MAX_LENGTH ||
    !parseRcsPublicWebhookUrl(callbackUrl)
  ) {
    return "";
  }
  return callbackUrl;
