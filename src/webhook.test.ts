...
import {
  ...
  resetRcsWebhookRateLimiterForTest,
  resetRcsWebhookReplayCacheForTest,
} from "./webhook.js";

const dispatchRcsInboundEvent = vi.hoisted(() =>
  vi.fn(async (_params: { msg: Record<string, unknown> }) => undefined),
);
...
describe("createRcsSharedTwilioWebhookHandler", () => {
  ...
  it("dispatches RCS payloads signed for the shared Twilio webhook URL", async () => {
    ...
    const handler = createRcsSharedTwilioWebhookHandler({
      cfg: {},
      account: createAccount(),
      channelRuntime: {} as RcsChannelRuntime,
